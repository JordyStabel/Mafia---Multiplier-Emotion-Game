...
namespace UnityEngine.Timeline
{
	public class VideoPlayableBehaviour : PlayableBehaviour
    {
...
#if UNITY_EDITOR // for scrubbing
                float compareTo = (float)((clipInTime + (playable.GetTime() * videoPlayer.playbackSpeed)) % videoPlayer.clip.length);
                if (!approximateFloats((float)videoPlayer.time, compareTo, 0.01f))
                {
                    SyncVideoToPlayable(playable);
                }
#endif
        ...
        private bool approximateFloats(float a, float b, float tolerance)
        {
